					...
					class CreateSeatPlan{
						...
						function __construct(){
    						...
    						if(isset($_GET['select_class'])){
      							echo '<p>Selected Classes: ' . implode(', ', $_GET['select_class']) . '</p>';
      							// $this->count_selected_class = count($_POST['select_class']);
      							if('select_class'=="Class 8"){

      							}
    						}
    						if(isset($_GET['select_room'])){
      							echo '<p>Selected Room: ' . implode(', ', $_GET['select_room']) . '</p>';
      							$a=implode(', ', $_GET['select_room']);
      							$count_selected_room = count($_GET['select_room']);
      							// echo "Total Selected Rooms: ".$count_selected_room;
      						}
    						if(isset($_GET['select_teacher'])){
      							echo '<p>Selected Teacher: ' . implode(', ', $_GET['select_teacher']) . '</p>';
      							$count_selected_teacher = count($_GET['select_teacher']);
      							// echo "Total Selected Teacher: ".$count_selected_teacher;
      						}
      				echo "<br>";
      						if(isset($_GET["Submit"])){
      							$select_class=$_GET['select_class'];
