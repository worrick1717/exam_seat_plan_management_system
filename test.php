...
<body>
  <?php
    require 'config.php';
    $calculate_to_display_students;
    $student_of_each_class=0;
    ...
    if(isset($_POST['select_class'])){
      ...
      echo "Total Selected Classes: " .$count_selected_class;
    }
    ...
    function roomno8seatplanning(){
            ...
            if(isset($_POST['Submit'])){
              ...
              foreach($selected_class as $value){
                if($value=="Class 8"){
                  $sql="SELECT s_class,roll_no,s_name FROM class_eight WHERE roll_no BETWEEN 1 AND {$student_of_each_class}";
                ...
                if($value=="Class 9"){
                  $sql="SELECT s_class,roll_no,s_name FROM class_nine WHERE roll_no BETWEEN 1 AND {$student_of_each_class}";
                  $dbresult=mysqli_query($con,$sql);
                  while($rows=mysqli_fetch_assoc($dbresult))
                  {
                    echo "<tr>";
                ...
                if($value=="Class 10"){
                  $sql="SELECT s_class,roll_no,s_name FROM class_ten WHERE roll_no BETWEEN 1 AND {$student_of_each_class}";
                  $dbresult=mysqli_query($con,$sql);
                  while($rows=mysqli_fetch_assoc($dbresult))
                  {
                    echo "<tr>";
    ...
    function roomno10seatplanning(){
            ...
            if(isset($_POST['Submit'])){
              ...
              foreach($selected_class as $value){
                ...
                if($value=="Class 9"){
                  $sql="SELECT s_class,roll_no,s_name FROM class_nine WHERE roll_no<{$student_of_each_class}";
                  $dbresult=mysqli_query($con,$sql);
                  while($rows=mysqli_fetch_assoc($dbresult))
                  {
                    echo "<tr>";
                ...
                if($value=="Class 10"){
                  $sql="SELECT s_class,roll_no,s_name FROM class_ten WHERE roll_no<{$student_of_each_class}";
                  $dbresult=mysqli_query($con,$sql);
                  while($rows=mysqli_fetch_assoc($dbresult))
                  {
                    echo "<tr>";
